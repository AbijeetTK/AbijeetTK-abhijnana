    ...
    btn: (v = 'ghost', col) => {
      if (v === 'primary')
        return {
          ...
          borderRadius: '8px',
          border: 'none',
          background: col || C.accent,
          color: col ? '#fff' : C.accentText,
          fontSize: '14px',
          fontWeight: 700,
          cursor: 'pointer',
          display: 'inline-flex',
      ...
      if (v === 'outline')
        return {
          padding: '9px 18px',
          borderRadius: '8px',
          border: `1.5px solid ${col || C.border}`,
...
function daysUntil(ds) {
  if (!ds) return null;
  return Math.ceil((new Date(ds) - Date.now()) / 86400000);
}
function uid() {
...
const DEMO_COURSES = [
  {
    ...
    progress: 45,
    lastStudied: Date.now() - 2 * 86400000,
    color: '#5a8fc0',
    ...
    resources: [
      {
        type: 'book',
        label: 'NISM Official Workbook',
        value: 'NISM Series XV Workbook 2024',
      },
  ...
  },
  {
    ...
    progress: 18,
    lastStudied: Date.now() - 5 * 86400000,
    color: '#8a72c8',
    examDate: new Date(Date.now() + 120 * 86400000).toISOString().split('T')[0],
    sections: [
      { name: 'Ethics', weight: 15, accuracy: 60 },
      { name: 'Quantitative Methods', weight: 8, accuracy: 45 },
      { name: 'Economics', weight: 8, accuracy: 0 },
      { name: 'Financial Reporting', weight: 15, accuracy: 0 },
      { name: 'Equity Investments', weight: 11, accuracy: 0 },
      { name: 'Fixed Income', weight: 11, accuracy: 0 },
      { name: 'Derivatives', weight: 6, accuracy: 0 },
      { name: 'Portfolio Management', weight: 8, accuracy: 0 },
    ],
    examPattern: {
      questions: 180,
      duration: 270,
    ...
    resources: [
      {
        type: 'book',
        label: 'CFA Institute Curriculum',
        value: 'CFA Program Curriculum 2024 Level I',
      },
      {
        type: 'website',
        label: 'CFA Institute',
      ...
      {
        type: 'video',
        label: 'Mark Meldrum YouTube',
        value: 'https://youtube.com/@markmeldrum',
      },
    ],
    notes: '',
    quizScores: [],
  },
  {
    ...
    examDate: null,
    sections: [
      { name: 'Listening', weight: 25, accuracy: 0 },
      { name: 'Reading', weight: 25, accuracy: 0 },
      { name: 'Writing', weight: 25, accuracy: 0 },
      { name: 'Speaking', weight: 25, accuracy: 0 },
    ],
    examPattern: {
      questions: null,
      duration: 120,
    ...
    resources: [
      {
        type: 'website',
        label: 'TV5Monde Exercises',
        value: 'https://apprendre.tv5monde.com',
      },
      {
        type: 'book',
        label: 'DELF A2 Preparation Book',
        value: 'DELF A2 200 activités',
      },
    ],
    notes: '',
    quizScores: [],
...
function AddCourseScreen({ onSave, onBack, editCourse, C, S }) {
  ...
  const [examType, setExamType] = useState(
    editCourse?.examPattern?.examType || 'MCQ'
  );
  const [questions, setQuestions] = useState(
    editCourse?.examPattern?.questions || ''
  );
  const [duration, setDuration] = useState(
    editCourse?.examPattern?.duration || ''
  );
  const [passScore, setPassScore] = useState(
    editCourse?.examPattern?.passScore || '60'
  );
  const [negMark, setNegMark] = useState(
    editCourse?.examPattern?.negativeMarking || '0'
  );
  ...
  const addSection = () => {
    ...
    setSections([
      ...sections,
      { name: newSec.trim(), weight: Number(newSecWeight) || 0, accuracy: 0 },
    ]);
    setNewSec('');
    setNewSecWeight('');
  };
  ...
  const addResource = () => {
    ...
    setResources([
      ...resources,
      { type: resType, label: resLabel.trim(), value: resValue.trim() },
    ]);
    setResLabel('');
    setResValue('');
  };
  ...
  const handleFinish = () => {
    const course = {
      ...
      syllabus,
      examPattern: {
        questions: Number(questions) || null,
        duration: Number(duration) || null,
        passScore: Number(passScore) || 60,
        negativeMarking: Number(negMark) || 0,
        examType,
      },
      topics:
        editCourse?.topics ||
  ...
  const canNext = [name.trim().length > 0, true, true, true, true][step];

  return (
    <div style={{ padding: '32px 28px', maxWidth: '600px', margin: '0 auto' }}>
      <button style={S.btn()} onClick={onBack} aria-label="Go back">
        ← Back
      </button>
      ...
      {step === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            ...
            />
            <p
              style={{ fontSize: '12px', color: C.textMuted, marginTop: '6px' }}
            >
              Can be a certification, language exam, degree module — anything
              you're studying for.
            </p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            ...
            <p
              style={{ fontSize: '12px', color: C.textMuted, marginTop: '6px' }}
            >
              The AI tutor will use this to suggest what to teach and in what
              order.
            </p>
          </div>
          ...
          <div>
            ...
            >
              Add each section of the exam with how much % it contributes to the
              final score.
            </p>
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '12px',
                    color:
                      sections.reduce((a, s) => a + s.weight, 0) === 100
                        ? C.green
                        : C.amber,
                    marginTop: '4px',
                  }}
                >
                  Total: {sections.reduce((a, s) => a + s.weight, 0)}%{' '}
                  {sections.reduce((a, s) => a + s.weight, 0) === 100
                    ? '✓'
            ...
            >
              ...
              <div style={{ width: '80px' }}>
                ...
                <input
                  ...
                  value={newSecWeight}
                  onChange={(e) => setNewSecWeight(e.target.value)}
                  placeholder="30"
                  onFocus={inputFocus}
                  onBlur={inputBlur}
                />
          ...
          <div>
            ...
            {examDate && daysUntil(examDate) !== null && (
              <p
                style={{
                  fontSize: '12px',
                  color: daysUntil(examDate) < 30 ? C.amber : C.green,
                  marginTop: '6px',
                }}
              >
              ...
              {EXAM_TYPES.map((t) => (
                <button
                  ...
                  style={{
                    padding: '7px 14px',
                    borderRadius: '20px',
                    border: `1.5px solid ${examType === t ? color : C.border}`,
                    background: examType === t ? color + '22' : 'transparent',
                    color: examType === t ? color : C.textMuted,
                    fontSize: '13px',
                    fontWeight: 600,
                    cursor: 'pointer',
                    fontFamily: 'inherit',
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                placeholder="0 = none, 0.25 = quarter"
                onFocus={inputFocus}
                onBlur={inputBlur}
              />
            </div>
          ...
          >
            ...
            >
              {[
                ...
                [
                  'Negative marking',
                  Number(negMark) > 0 ? `-${negMark} per wrong` : 'None',
                ],
              ].map(([k, v]) => (
                <div
                  key={k}
      ...
      {step === 3 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <p style={{ fontSize: '14px', color: C.textMuted, margin: 0 }}>
            Add all the study material you'll be using — books, websites,
            YouTube channels, PDFs, etc. The AI tutor will know what sources
            you're working from.
          </p>
                ...
                {RESOURCE_TYPES.map((rt) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: '20px',
                      border: `1.5px solid ${
                        resType === rt.key ? color : C.border
                      }`,
                      background:
                        resType === rt.key ? color + '22' : 'transparent',
                      color: resType === rt.key ? color : C.textMuted,
                      fontSize: '13px',
                      cursor: 'pointer',
                      fontFamily: 'inherit',
                      display: 'flex',
            ...
            >
              <div>
                ...
                <input
                  ...
                  placeholder={
                    resType === 'website'
                      ? 'e.g. SEBI Official Website'
                      : resType === 'video'
                      ? 'e.g. Mark Meldrum CFA'
                      : resType === 'book'
                      ? 'e.g. CFA Institute Curriculum'
                      : 'Label'
                  }
                  onFocus={inputFocus}
                  onBlur={inputBlur}
              ...
              <div>
                <label style={S.label}>
                  {resType === 'website' || resType === 'video'
                    ? 'URL / link'
                    : resType === 'document'
                    ? 'File name or paste content'
                    : 'Title / ISBN / details'}
                </label>
                <input
                  style={S.input}
                  ...
                  placeholder={
                    resType === 'website'
                      ...
                      ? 'https://youtube.com/…'
                      : resType === 'notes'
                      ? 'Paste your notes here or describe them'
                      : 'e.g. NISM Series XV Workbook 2024'
                  }
                  onFocus={inputFocus}
                  onBlur={inputBlur}
                />
              ...
              <button
                style={{ ...S.btn('outline', color), alignSelf: 'flex-start' }}
                onClick={addResource}
              >
                + Add resource
              </button>
          ...
          {resources.length === 0 && (
            ...
            >
              No resources added yet — you can skip this and add later.
            </p>
          )}
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            ...
            >
              <p style={{ fontSize: '12px', color: C.textMuted }}>
                This is the AI tutor's source of truth. The more you add, the
                better it teaches.
              </p>
          ...
          {notes.length === 0 && (
            ...
            >
              ...
              >
                Without study material, the AI tutor will use general knowledge.
                It will clearly flag this so you always know.
              </p>
            </div>
          )}
          ...
          {notes.length > 0 && (
            ...
            >
              ...
              >
                The AI tutor will teach strictly from your notes — no outside
                knowledge added.
              </p>
            </div>
          )}
          ...
          >
            ...
            {[
              ...
              [
                'Sections',
                sections.length > 0
                  ? sections.map((s) => `${s.name} (${s.weight}%)`).join(', ')
                  : 'None added',
              ],
              [
                'Exam date',
                examDate
                  ? `${examDate} (${daysUntil(examDate)} days)`
                  : 'Not set',
              ],
              ['Exam type', examType],
              [
                'Resources',
                resources.length > 0 ? `${resources.length} added` : 'None',
              ],
              [
                'Study notes',
    ...
    <div style={{ padding: '32px 28px', maxWidth: '920px', margin: '0 auto' }}>
      ...
      >
        <div>
          ...
          <p
            style={{ margin: '4px 0 0', color: C.textMuted, fontSize: '14px' }}
          >
            Welcome back, Abhijeet · {courses.length} course
            {courses.length !== 1 ? 's' : ''}
          </p>
        </div>
        <button style={S.btn('primary')} onClick={onAdd}>
          + Add Course
        </button>
      </div>
      ...
      {courses.length === 0 && (
        <div style={{ textAlign: 'center', padding: '80px 20px' }}>
          ...
          <p style={{ fontSize: '17px', color: C.text, marginBottom: '8px' }}>
            No courses yet
          </p>
          <p style={{ color: C.textMuted }}>
            Hit "+ Add Course" to get started
          </p>
        </div>
      )}
            ...
            >
                      ...
                      {[
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: 'none',
                            border: 'none',
                            color: lbl.includes('Delete') ? C.red : C.text,
                            textAlign: 'left',
                            cursor: 'pointer',
                            fontSize: '13px',
                            fontFamily: 'inherit',
              ...
              <div style={{ marginBottom: '14px' }}>
                ...
                >
                  ...
                  {days !== null && (
                    <span style={S.tag(days < 30 ? C.amber : C.blue)}>
                      {days}d to exam
                    </span>
                  )}
                </div>
              ...
              >
                <span style={{ fontSize: '12px', color: C.textMuted }}>
                  Last studied: {timeAgo(c.lastStudied)}
                </span>
                {c.sections.length > 0 && (
                  <span style={S.pill}>{c.sections.length} sections</span>
                )}
              </div>
  ...
  return (
    <div style={{ padding: '28px', maxWidth: '760px', margin: '0 auto' }}>
      ...
      >
        ...
        <button
          style={{ ...S.btn('outline'), fontSize: '13px', color: C.textMuted }}
          onClick={onEdit}
        >
          ✏️ Edit course
        </button>
      ...
      <div style={{ textAlign: 'center', marginBottom: '32px' }}>
        ...
        >
          ...
          {course.examPattern?.examType && (
            <span style={S.pill}>{course.examPattern.examType}</span>
          )}
          {course.examPattern?.questions && (
            <span style={S.pill}>{course.examPattern.questions} questions</span>
          )}
          {course.examPattern?.passScore && (
            <span style={S.pill}>Pass: {course.examPattern.passScore}%</span>
          )}
        </div>
      </div>
        ...
        {[
          {
            label: 'Learn',
            icon: '📖',
            desc: 'AI explains topics to you',
            fn: onLearn,
            color: C.blue,
          },
          {
      ...
      {weak.length > 0 && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {weak.map((s) => (
              <span key={s.name} style={S.tag(C.amber)}>
                {s.name} · {s.accuracy}%
              </span>
            ))}
          </div>
...
function LearnScreen({ course, topic, onBack, onQuiz, C, S }) {
  ...
  const callAI = useCallback(
    async (userMsg, isIntro = false) => {
      setLoading(true);
      const hasMaterial = course.notes && course.notes.trim().length > 20;
      const system = `You are a warm, friendly personal tutor teaching "${
        course.name
...
${
  hasMaterial
    ? `Teach ONLY from this material. Do not add anything outside it:\n\n---\n${course.notes}\n---`
...
Rules: End with ONE comprehension question. Keep it under 250 words. Be warm and encouraging. Never say "as an AI".`;
      ...
      const msgs = isIntro
        ? [
            {
              role: 'user',
              content: `Introduce and explain: ${
                topic ? topic.name : 'the first important topic of this course'
              }. Start directly.`,
            },
          ]
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          style={{ ...S.btn(), color: C.amber, fontSize: '13px' }}
          onClick={onQuiz}
        >
          Quiz me →
        </button>
          ...
          >
            ...
            <div
              style={{
                ...
                border: `1px solid ${
                  m.role === 'user' ? C.accent + '50' : C.border
                }`,
                fontSize: '15px',
                lineHeight: 1.7,
                color: C.text,
      ...
      >
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
            <button
              style={{ ...S.btn(), fontSize: '13px' }}
              onClick={() => callAI('Got it! Move to the next topic please.')}
            >
              Got it, next →
            </button>
          </div>
...
function QuizScreen({ course, topic, onBack, C, S }) {
  ...
  const generate = async () => {
    ...
    setAnswers([]);
    setSelected(null);
    const sys = `Generate 5 MCQs about "${
      topic ? topic.name : course.name
    }" for "${course.name}".
${
  course.notes
...
[{"question":"...","options":["A. ...","B. ...","C. ...","D. ..."],"correct":0,"explanation":"..."}]`;
  ...
  if (phase === 'loading')
    return (
      ...
      >
        <Spinner C={C} />
        <p style={{ color: C.textMuted }}>Generating quiz…</p>
      </div>
    );
  if (phase === 'error')
    return (
      <div style={{ padding: '40px', textAlign: 'center' }}>
        <p style={{ color: C.red, marginBottom: '16px' }}>
          Couldn't generate quiz.
        </p>
        <button style={S.btn('primary')} onClick={generate}>
          Try again
        </button>
        <button style={{ ...S.btn(), marginLeft: '12px' }} onClick={onBack}>
          Back
        </button>
      </div>
    );
  ...
  if (phase === 'results') {
    ...
    return (
      <div
        style={{ padding: '32px 28px', maxWidth: '600px', margin: '0 auto' }}
      >
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div
            style={{
          ...
          <>
            ...
            {wrong.map(({ q, sel }, i) => (
              ...
              >
                ...
                <p
                  style={{ margin: '0 0 4px', fontSize: '13px', color: C.red }}
                >
                  You chose: {q.options[sel]}
                </p>
                <p
                  style={{
                    margin: '0 0 8px',
...
Return ONLY valid JSON: [{"front":"term","back":"explanation in 1-2 sentences"}]`;
  ...
  if (loading)
    return (
      ...
      >
        <Spinner C={C} />
        <p style={{ color: C.textMuted }}>Generating flashcards…</p>
      </div>
    );
  if (!cards.length)
    return (
      <div style={{ padding: '40px', textAlign: 'center' }}>
        <p style={{ color: C.red }}>Couldn't generate cards.</p>
        <button style={S.btn('primary')} onClick={generate}>
          Retry
        </button>
      </div>
    ...
    <div style={{ padding: '28px', maxWidth: '540px', margin: '0 auto' }}>
      ...
      <div
        ...
        style={{
          ...
          minHeight: '230px',
          background: C.card,
          border: `1px solid ${flipped ? C.accent + '80' : C.border}`,
          borderRadius: '20px',
          padding: '40px 32px',
          cursor: 'pointer',
          display: 'flex',
          ...
          userSelect: 'none',
          background: flipped ? C.accent + '0a' : C.card,
        }}
      >
        <span
          style={{
        ...
        >
          {flipped ? 'Answer' : 'Concept'}
        </span>
        <p
          style={{
            margin: 0,
            fontSize: flipped ? '15px' : '20px',
        ...
        {!flipped && (
          <p
            style={{ margin: '18px 0 0', fontSize: '12px', color: C.textMuted }}
          >
            Tap to reveal
          </p>
        )}
      ...
      >
        <button
          ...
          onClick={() => {
            setFlipped(false);
            setTimeout(() => setIdx(Math.max(0, idx - 1)), 150);
          }}
        >
          ←
        </button>
...
[{"question":"...","options":["A. ...","B. ...","C. ...","D. ..."],"correct":0,"explanation":"...","section":"..."}]`;
  ...
  if (phase === 'pre')
    return (
      <div
        style={{ padding: '32px 28px', maxWidth: '520px', margin: '0 auto' }}
      >
        <button style={{ ...S.btn(), marginBottom: '24px' }} onClick={onBack}>
          ← Back
        </button>
        <h2
          style={{
        ...
        >
          ...
          {[
            [
              'Questions',
              ep.questions
                ? `${Math.min(ep.questions, 20)} (capped at 20 for practice)`
                : '20',
            ],
            ['Duration', ep.duration ? `${ep.duration} min` : '30 min'],
            ['Pass score', `${ep.passScore || 60}%`],
            [
              'Negative marking',
              ep.negativeMarking > 0
                ? `-${ep.negativeMarking} per wrong`
                : 'None',
            ],
          ].map(([k, v]) => (
            <div
  ...
  if (phase === 'exam') {
    ...
    return (
      <div
        style={{ padding: '24px 28px', maxWidth: '600px', margin: '0 auto' }}
      >
        <div
          style={{
            display: 'flex',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {q.options.map((opt, i) => (
            <button
              ...
              style={{
                padding: '14px 18px',
                background: selected === i ? course.color + '22' : C.card,
                border: `1px solid ${selected === i ? course.color : C.border}`,
                borderRadius: '12px',
                textAlign: 'left',
                cursor: 'pointer',
                fontSize: '14px',
  ...
  if (phase === 'results') {
    ...
    const wrong = answers
      ...
      .filter(({ a, q }) => a !== q?.correct && a !== null);
    return (
      <div
        style={{ padding: '32px 28px', maxWidth: '620px', margin: '0 auto' }}
      >
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div
            style={{
            ...
            {wrong.slice(0, 6).map(({ a, q }, i) => (
              ...
              >
                ...
                <p
                  style={{ margin: '0 0 4px', fontSize: '13px', color: C.red }}
                >
                  You: {a !== null ? q.options[a] : 'Skipped'}
                </p>
                <p
                  style={{
                    margin: '0 0 8px',
...
export default function App() {
  ...
  const openCourse = (id) => {
    ...
    setActiveTopic(null);
    setCourses((prev) =>
      prev.map((c) => (c.id === id ? { ...c, lastStudied: Date.now() } : c))
    );
    setScreen('course');
  };
      ...
      <div style={{ overflowY: 'auto', height: 'calc(100vh - 60px)' }}>
        ...
        {screen === 'add' && (
          <AddCourseScreen
            ...
            onBack={() => {
              setEditingCourse(null);
              setScreen(editSource === 'course' ? 'course' : 'home');
            }}
        ...
        {screen === 'course' && activeCourse && (
          <CourseHomeScreen
            course={activeCourse}
            onLearn={(topic) => {
              setActiveTopic(topic?.id ? topic : null);
              setScreen('learn');
            }}
            onQuiz={() => {
              setActiveTopic(null);
